...
export function createFakeAuthAdapter(): AuthAdapter {
  ...
  return { async signIn(_e, _p) { session = { accessToken: "fake", user: { id: "u1", email: "dev@example.com" } }; return session; }, async signOut() { session = null; }, async getSession() { return session; } };
}
