...
/**
 * Built-output link validation: Crawl out/ for broken internal links.
 */
...
describe('Built-Output Internal Links', () => {
    ...
    for (const file of htmlFiles) {
      ...
      $('a[href]').each((_, el) => {
        ...
        if (!normalizedHref.startsWith('/')) {
          const fileRelDir = file.replace(outDir, '').replace('/index.html', '/');
          normalizedHref = resolve(fileRelDir, normalizedHref).replace(/\/+/g, '/');
        }

        // Check if path resolves to a file in out/
        const possiblePaths = [
          normalizedHref,
        ...
        const resolvedPaths = possiblePaths.map((p) => join(outDir, p));
  ...
  it('sitemap.xml exists in out/', () => {
    const hasSitemap =
      existsSync(join(outDir, 'sitemap.xml')) ||
      existsSync(join(outDir, 'sitemap-index.xml')) ||
      existsSync(join(outDir, 'sitemap-0.xml'));
