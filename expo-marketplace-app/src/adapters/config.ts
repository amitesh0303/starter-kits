export interface AppConfig { supabaseUrl: string | null; supabaseAnonKey: string | null; stripeKey: string | null; algoliaAppId: string | null; algoliaSearchKey: string | null; googleMapsKey: string | null; isFakeMode: boolean; }
...
export function validateConfig(): ConfigValidationResult {
  ...
  const algoliaAppId = getEnvVar("EXPO_PUBLIC_ALGOLIA_APP_ID") || null;
  const algoliaSearchKey = getEnvVar("EXPO_PUBLIC_ALGOLIA_SEARCH_KEY") || null;
  ...
  return { valid: true, errors: [], warnings: w, config: { supabaseUrl, supabaseAnonKey, stripeKey, algoliaAppId, algoliaSearchKey, googleMapsKey, isFakeMode } };
}
